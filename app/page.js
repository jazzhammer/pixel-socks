  ...
  return (
    <main className="flex min-h-screen flex-col items-center">
      <div className={"text-xl font-bold flex flex-row"}>
        <div>pixel-socks</div>
        <div><Image className={"w-4 pt-1 ml-2"} src={sock} alt={"sock"}/></div>
      </div>
      <div className={"mt-24 flex flex-row"}>
        <div className={"mr-6 mt-9"}>select a color -&gt;</div>
        <div className={"mr-6 mt-8"}>
          <ColorPicker onChange={selectColor}/>
        </div>
        <div>
          <canvas height={100} width={100} style={{background: 'white'}} onClick={clickCanvas}></canvas>
        </div>
        {color && <div className={"ml-6 mt-9"}>&lt;- drop a pixel</div>}
      </div>
    </main>
  )
}
